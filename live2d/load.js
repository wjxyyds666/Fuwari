...
function onModelLoad(model) {
  ...
  // 修正拼写错误：modelNmae → modelName
  const modelName = model.internalModel.settings.name
  const coreModel = model.internalModel.coreModel
  const motionManager = model.internalModel.motionManager

  let touchList = [
    { text: "点击展示文本1", motion: "Idle" },
    { text: "点击展示文本2", motion: "Idle" }
  ]
  ...
  if (modelName === "Diana") {
    ...
    const hideParts = [
      "Part5", "neko", "game", "Part15", "Part21", "Part22", "Part", "Part16", "Part12"
    ]
